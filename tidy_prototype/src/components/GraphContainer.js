...
import { COLORS, CURSOR_STATES } from '../constants.js'

const GraphContainer = ({ rect, graphContainerId, children, cursorStateType }) => {
	...
	return (
		<g
			onMouseMove={e => {
				const cursorStateType = _.get(stateManager.state, 'uiState.cursorState.stateType')
				if (!cursorStateType || cursorStateType === CURSOR_STATES.addContainer) {
			...
			<MoveArrowIcon
				onMouseDown={e =>
					dragManager.start(e, {
						...
						onDrag(_, { dx, dy }) {
							stateManager.setState(state => {
								state.graphContainers[graphContainerId].pos = new Pos(
									this.rootX + dx,
									this.rootY + dy
								)
							})
			...
			<DeleteIcon
				...
				size={moveIconSize}
			/>
			<Rect rect={rect} borderColor={COLORS.lightGrey} />
			{children}
		</g>
	)
}
